...
// Searches for all pull requests where the specified user is requested as a reviewer
func (client *GitHubClient) ListPullRequestsByReviewer(ctx context.Context, username string) ([]PullRequestInfo, error) {
	client.logger.Debug("Fetching pull requests with review requested for", username, "across all repositories")

	query := fmt.Sprintf("type:pr state:open review-requested:%s", username)
	client.logger.Debug("GitHub Search Query:", query)
	...
	for {
		...
		if err != nil {
			client.logger.Debug("GitHub Search Error:", err)
			return []PullRequestInfo{}, err
		}

		client.logger.Debug(fmt.Sprintf("GitHub Search returned %d results (page %d)", len(result.Issues), opts.Page))
		allIssues = append(allIssues, result.Issues...)
	...
	client.logger.Debug(fmt.Sprintf("Total PRs found for reviewer %s: %d", username, len(allIssues)))
